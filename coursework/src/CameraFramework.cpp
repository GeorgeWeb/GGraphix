...
namespace SpookyTown
{
	...
	{
		// starting camera option
		set_free_camera();
	}

	free_camera &CameraFramework::get_free_camera() { return freeCam; }

	chase_camera &CameraFramework::get_chase_camera() { return chaseCam; }

	void CameraFramework::set_free_camera()
	{
		freeCam.set_position(vec3(0.0f, 50.0f, 0.0f));
	...
	void CameraFramework::set_chase_camera(mesh chaseObj)
	{
	...
	void CameraFramework::rotate_free_camera(double cursor_x, double cursor_y)
	{
	...
	void CameraFramework::rotate_chase_camera(double cursor_x, double cursor_y)
	{
		// The ratio of pixels to rotation - remember the fov
		static double ratio_width = quarter_pi<float>() / static_cast<float>(renderer::get_screen_width());
		static double ratio_height = (quarter_pi<float>() * (static_cast<float>(renderer::get_screen_height()) /
	...
	void CameraFramework::move_free_camera(vec3 translate)
	{
		// Move camera
		freeCam.move(translate);
	}

	void CameraFramework::update_free_camera(float delta_time)
	{
		// Update the camera
		freeCam.update(delta_time);
	}

	void CameraFramework::update_chase_camera(float delta_time)
	{
