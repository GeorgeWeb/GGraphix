...
texture::texture(const std::string &filename) throw(...) : texture(filename, true, true, false) {}

// Creates a new texture object from the given file with mipmaps and anisotropic filtering defined
texture::texture(const std::string &filename, bool mipmaps, bool anisotropic, bool gammaCorrection) throw(...) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, gammaCorrection ? GL_SRGB : GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, &pixel_data[0]);
